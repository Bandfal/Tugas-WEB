<?php
include "connect.php";

if(isset($_POST['update'])) {
    $id = $_POST['id'];
    $nama_siswa = $_POST['nama_siswa'];
    $foto = $_POST['foto'];
    $kelas_siswa = $_POST['kelas_siswa'];
    $jurusan_siswa = $_POST['jurusan_siswa'];
    
    $data = mysqli_query($connect, "UPDATE siswa SET nama_siswa='$nama_siswa', kelas_siswa='$kelas_siswa', jurusan_siswa='$jurusan_siswa' WHERE id='$id'");

    header("Location: lihat_data.php");
} else {
    
}
$id = $_GET['id']; 

$data = mysqli_query($connect, "SELECT * FROM siswa WHERE id='$id'");

while ($data_siswa = mysqli_fetch_array($data)) {
    $id = $data_siswa['id'];
    $nama_siswa = $data_siswa['nama_siswa'];
    $foto = $data_siswa['foto'];
    $kelas_siswa = $data_siswa['kelas_siswa'];
    $jurusan_siswa = $data_siswa['jurusan_siswa'];
}

?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Edit Data</title>
    <link rel="stylesheet" href="style_form.css">
</head>
<body>
    <form action="edit_data.php" method="post" enctype="multipart/form-data">
        <div>
            <label for="nama_siswa">Masukkan Nama Siswa: </label>
            <input type="text" name="nama_siswa" value="<?php echo $nama_siswa; ?>">
        </div>
        <div>
            <label for="foto">Masukkan Foto Siswa: </label>
            <input type="file" name="foto" value="<?php echo $foto; ?>">
            <div></div>
        </div>
        <div>
            <label for="kelas_siswa">Masukkan Kelas Siswa: </label>
            <select name="kelas_siswa">
                <option <?= ($kelas_siswa == 1)? "selected" : ""?> value="1">XI RPL 1</option>
                <option <?= ($kelas_siswa == 2)? "selected" : ""?> value="2">XI RPL 2</option>
                <option <?= ($kelas_siswa == 3)? "selected" : ""?> value="3">XI MM 1</option>
                <option <?= ($kelas_siswa == 4)? "selected" : ""?> value="4">XI MM 2</option>
                <option <?= ($kelas_siswa == 5)? "selected" : ""?> value="5">XI TKJ 1</option>
                <option <?= ($kelas_siswa == 6)? "selected" : ""?> value="6">XI TKJ 2</option>
            </select>
        </div>
        <div>
            <label for="jurusan_siswa">Masukkan Jurusan Siswa: </label>
            <select name="jurusan_siswa">
                <option <?= ($jurusan_siswa == 1)? "selected" : ""?> value="1">Rekayasa Perangkat Lunak</option>
                <option <?= ($jurusan_siswa == 2)? "selected" : ""?> value="2">Multimedia</option>
                <option <?= ($jurusan_siswa == 3)? "selected" : ""?> value="3">Teknik Komputer dan Jaringan</option>
            </select>
        </div>
        <td><input type="hidden" name="id" value=<?php echo $id;?>></td>
        <td><input type="submit" name="update" value="Update"></td>
    </form>
</body>
</html>